...
export default function GameCanvas() {
  ...
  for (let x = 0; x <= 2000; x += 20) {
    gridLines.push(<line key={`vx-${x}`} x1={x} y1={0} x2={x} y2={2000} stroke="#1e293b" strokeWidth={1} />);
  }

  for (let y = 0; y <= 2000; y += 20) {
    gridLines.push(<line key={`hy-${y}`} x1={0} y1={y} x2={2000} y2={y} stroke="#1e293b" strokeWidth={1} />);
  }
        ...
        <svg width="100%" height="100%" onClick={handleCanvasClick}>
          ...
          {nodes.map(node => (
            <circle
              ...
              cy={node.y}
              r={7}
              fill={selectedNodeId === node.id ? '#facc15' : '#60a5fa'}
              onClick={(e) => {
                e.stopPropagation();
